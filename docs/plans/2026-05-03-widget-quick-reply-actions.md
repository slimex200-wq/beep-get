...
This keeps the required product behavior visible and testable while avoiding a premature native auth/session/idempotency design.
...
Native direct-send from the widget is intentionally deferred until shared native auth, retry handling, and server-side idempotency exist.
...
- Actual placed Android launcher widget screenshot still needs manual launcher placement.
- iOS WidgetKit implementation and verification are not covered by this Windows pass.
- True one-tap background sends should wait for native auth/session sharing and DB idempotency.
