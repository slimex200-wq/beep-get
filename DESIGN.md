...
- 2026-07-02 navigation semantics: three transition grammars, matched to affordances — (1) **push** for chevron-row drill-ins (My → Dictionary/Logs/Account; header gets a left back arrow via `Screen.onBack`, never a modal X), (2) **modal screen** only for compose tasks (Send-back from friend rows; ReplyRoom stays a pushed Today detail), (3) **bottom sheet** for every transient pick/config task (skin pack, avatar, quick-reply slots, add friend — MyScreen's local `SheetShell` pattern: slide-up, scrim tap-to-close, grab bar, top-only radius; centered fade dialogs are banned). Locked by rootNavigator/MyScreen/PeopleScreen test contracts.
- 2026-07-02 UI rebuild (component DNA swap): every primary and utility surface (Today/Send/People/My + Settings/Dictionary/Logs) was reassembled on the new design system `src/ui/primitives.tsx` (Screen/SectionLabel/Card/ListRow/Chip/PillButton/PrimaryButton/Segmented/Beacon/StatusDot/SignalKindLabel/Perforation/MonoValue) — primitives own the color rules so screens cannot drift. Data wiring (stores/services/lib), native widget modules/targets, assets, `appTheme` + `themeStore.signalColor`, and `FixedTabBar` were kept; the Kotlin-mockup component grammar was retired from all rebuilt screens. Deleted legacy: Apple liquid-glass preview feature (screens/components/lib/index.js branch), SendBeepScreen/SendBlinkScreen (unregistered modal legacy), StudioScreen/CollectionScreen, TodayMockupChrome/TodayIncomingCard/SendMockup*/FriendPulseCard/CloseCircuitMap/my-cards/settings-rows. Quick-reply defaults replaced: `["Done","8282","View"]` → `["OK","지금가","콜"]` ("Done" kept as read-action special case). Remaining legacy island: `KotlinMockupUI.tsx` still serves SlipReplyRoomScreen (IconButton/StatusPill) and the unregistered WidgetStatesScreen dev preview — port those two to primitives to delete it. Verified: typecheck + Jest 453/453 (76 suites) + expo-web screenshots pixel-consistent with the polished Signal Edition pass. Platform gap unchanged: web/Jest only.
- 2026-07-02 Signal Edition (`mockup-signal-2026-07-02.html`, interactive; also `mockup-live-2026-07-02.html` = purple v2 for comparison): visual-language layer on top of the Quiet Pager reset — "Paper · Ink · Signal". Primary actions are ink (near-black in light, cream-inverted in dark); one **signal color** is reserved exclusively for signal semantics (LED beacons, INCOMING labels, REC, recipient selection ring, new-signal codes); the Today hero is a perforated paper-slip card (punch-hole perforation divider = signature element); all chips/segments/inputs are pills; section labels carry a signal-color square tick; nav active state is ink + signal dot. Signal color is a **user setting** in My (`Signal Color`: Signal Orange `#FF4E1F` / Electric Violet `#7C3AED`, dark variants `#FF5A26`/`#A78BFA`) — both ship; it tints app chrome signal semantics while skin packs keep owning widget surfaces and the app light/dark theme stays a separate system toggle. Supersedes the pastel-purple-as-primary direction from 2026-06-09. **Status: implemented 2026-07-02** — `appTheme.ts` (ink primary + `sig`/`sigSoft` tokens, `SIGNAL_COLOR_OPTIONS`), `themeStore.signalColor` (SecureStore-persisted), `FixedTabBar` replaces the liquid-glass expandable bar (SAVED/ACCOUNT/CODES moved to My rows), Today hero = perforated slip with Widget Mirror removed, Send/People/My restyled, My owns the Signal Color picker, AuthScreen demo accents aligned to Signal Orange + Signal paper. Verified: typecheck + Jest 446/446 + expo-web screenshots (light/dark, orange/violet). Platform gap: iOS/Android device screenshots not taken (web-only QA in this workspace).
- 2026-07-02 reset proposal (`mockup-reset-2026-07-02.html`, captures `qa-capture/mockup-reset-light.png` / `mockup-reset-dark.png`): "Quiet Pager" full UI reset. Diagnosis: per-screen visual drift (ivory Today vs black "My Beep Room" vs lavender Send), concept inflation (Friend Pulse / Widget Mirror / Room Style / Photo Avatar / expandable More nav), duplicated latest-signal display on Today, and noisy dual left/right section labels. Reset rules: one background per mode, one card style, one purple accent, left-only section labels, fixed 4-tab bottom bar (liquid-glass expandable nav removed; overflow items move under My), and the Today hero card adopts the widget grammar directly so Widget Mirror is deleted. User concepts reduced to two: Signal and Friends. Supersedes the 2026-06-09 liquid-glass expandable nav direction if implemented. Status: mockup approved-pending; RN implementation not started.
